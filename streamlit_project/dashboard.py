...
from utils import read_pdf_upload, save_json
    ...
    if 'analysis' in st.session_state:
        ...
        if st.button("💾 Save Configuration", use_container_width=True):
            ...
            if save_json(config_data, config_filename):
                ...
                print(f"✅ [SERVER] Configuration saved to: {config_filename}")
            ...
            st.download_button(
                ...
                data=config_json,
                file_name=f"sales_config_{config_timestamp}.json",
                mime="application/json",
                use_container_width=True
            )
        ...
        saved_files = sorted([f for f in os.listdir(RESULTS_DIR) if f.endswith('.json')], reverse=True)
        
        if saved_files:
            st.caption(f"Found {len(saved_files)} saved analysis file(s)")
            
            # Show most recent 5 files
            for filename in saved_files[:5]:
                file_path = os.path.join(RESULTS_DIR, filename)
                file_time = datetime.fromtimestamp(os.path.getmtime(file_path))
                st.text(f"📄 {filename} ({file_time.strftime('%Y-%m-%d %H:%M')})")
        else:
            ...
            with st.expander("📊 Yearly Cost Breakdown"):
                ...
                for year_str in [str(y) for y in years]:
                    ...
                    table_data.append({
                        ...
                        'Development': f"€{year_data['one_time_development']:,.0f}",
                        'CAC': f"€{year_data['customer_acquisition']:,.0f}",
                        'Distribution': f"€{year_data['distribution_operations']:,.0f}",
                        'After Sales': f"€{year_data['after_sales']:,.0f}",
                        'COGS': f"€{year_data['total_cogs']:,.0f}",
                        'COGS/Unit': f"€{year_data['cogs_per_unit']:.2f}",
                        'Total Cost': f"€{year_data['total_cost']:,.0f}"
                    })
                    ...
                    for year_str in [str(y) for y in years]:
                        ...
                        with st.expander(f"Year {year_str} Details"):
                            ...
                            with col1:
                                ...
                                st.metric("💻 Development", f"€{year_data['one_time_development']:,.0f}",
                                         help="One-time cost in first year only")
                                st.metric("📢 CAC", f"€{year_data['customer_acquisition']:,.0f}",
                                         help="Annual customer acquisition cost")
                            with col2:
                                st.metric("🚚 Distribution", f"€{year_data['distribution_operations']:,.0f}",
                                         help="Annual distribution & operations cost")
                                st.metric("🛠️ After Sales", f"€{year_data['after_sales']:,.0f}",
                                         help="Annual after sales cost")
                                st.metric("💰 Total COGS", f"€{year_data['total_cogs']:,.0f}",
                                         help="Auto-calculated: Volume × COGS per unit")
                            
                            st.divider()
                            col_a, col_b = st.columns(2)
                            with col_a:
                                st.metric("🔢 COGS per Unit", f"€{year_data['cogs_per_unit']:.2f}")
                            with col_b:
                                st.metric("💵 Total Cost", f"€{year_data['total_cost']:,.0f}",
                            ...
                            **Calculation Breakdown:**
                            ```
                            Development:     €{year_data['one_time_development']:>15,}
                            CAC:            €{year_data['customer_acquisition']:>15,}
                            Distribution:   €{year_data['distribution_operations']:>15,}
                            After Sales:    €{year_data['after_sales']:>15,}
                            COGS:           €{year_data['total_cogs']:>15,}
                                            ({year_data['projected_volume']:,} × €{year_data['cogs_per_unit']:.2f})
                            ────────────────────────────────
                            Total:          €{year_data['total_cost']:>15,}
                            ```
                            """)
                ...
                if 'development_costs' in cost_data:
                    ...
                    for cost_item in cost_data['development_costs']:
                        with st.container():
                            ...
                            with col2:
                                st.metric("Estimated Cost", f"€{cost_item['estimated_amount']:,}")
                            ...
                            if 'market_comparison' in cost_item:
                                ...
                                if 'cost_figures' in mc and mc['cost_figures']:
                                    ...
                                    for fig in mc['cost_figures']:
                                        st.text(f"• {fig['company']} - {fig['project']}: €{fig['amount']:,} ({fig['year']})")
                    ...
                    for cost_item in cost_data['customer_acquisition_costs']:
                        with st.container():
                            ...
                            with col1:
                                ...
                                st.write(cost_item['reasoning'])
                            with col2:
                                st.metric("Annual Budget", f"€{cost_item.get('estimated_annual_budget', 0):,}")
                                if 'estimated_amount_per_customer' in cost_item:
                                    st.metric("Cost per Customer", f"€{cost_item['estimated_amount_per_customer']}")
                ...
                if 'cost_of_goods_sold' in cost_data:
                    ...
                    for cost_item in cost_data['cost_of_goods_sold']:
                        with st.container():
                            ...
                            with col2:
                                st.metric("Price per Item", f"€{cost_item.get('price_per_item', 0)}")
                                st.metric("COGS per Item", f"€{cost_item.get('cogs_per_item', 0)}")
                            with col3:
        ...
        if not st.session_state.income_analysis_done:
            ...
            if st.session_state.income_analysis:
                ...
                with st.expander("🔍 Debug: Data Structure", expanded=False):
                    st.write("**Type of yearly_projections:**", type(yearly_projections))
                    st.write("**Length:**", len(yearly_projections) if yearly_projections else 0)
                    if yearly_projections:
                        st.write("**First item type:**", type(yearly_projections[0]) if len(yearly_projections) > 0 else "N/A")
                        st.write("**First item:**", yearly_projections[0] if len(yearly_projections) > 0 else "N/A")
                    st.write("**Full income_data keys:**", list(income_data.keys()))
                    st.json(income_data)
                ...
                if yearly_projections:
                    ...
                    def format_large_number(value):
                        """Format large numbers with M for millions, B for billions"""
                        abs_value = abs(value)
                    ...
                    if yearly_projections and len(yearly_projections) > 0 and all(isinstance(proj, dict) for proj in yearly_projections):
                        ...
                        if st.session_state.selected_income_strategy == "Royalties":
                            ...
                            with col2:
                                st.metric("💰 Price per Unit", f"€{first_year_data.get('price_per_piece', 0):,.2f}")
                            with col3:
                            ...
                            col1, col2, col3, col4 = st.columns(4)
                            with col1:
                                st.metric("📅 Monthly Cost", f"€{first_year_data.get('monthly_cost', 0):,.2f}")
                            with col2:
                                st.metric("📈 Annual Revenue", f"€{first_year_data.get('monthly_cost', 0) * 12:,.2f}",
                                         help="Per subscriber")
                            with col3:
                            ...
                            for year_data in yearly_projections:
                                ...
                                if st.session_state.selected_income_strategy == "Royalties":
                                    table_data.append({
                                        ...
                                        'Price/Unit': f"€{year_data.get('price_per_piece', 0):,.2f}",
                                        'Royalty %': f"{year_data.get('royalty_percentage', 0)}%",
                                        'Yearly Income': f"€{year_data.get('yearly_income', 0):,.2f}"
                                    })
                                elif st.session_state.selected_income_strategy == "Subscription":
                                    table_data.append({
                                        'Year': year,
                                        'Subscribers': f"{year_data.get('number_of_subscribers', 0):,}",
                                        'Monthly Cost': f"€{year_data.get('monthly_cost', 0):,.2f}",
                                    ...
                                    table_data.append({
                                        'Year': year,
                                        'Units Sold': f"{year_data.get('number_of_units', 0):,}",
                                        'Price': f"€{year_data.get('price', 0):,.2f}",
                                        'Yearly Income': f"€{year_data.get('yearly_income', 0):,.2f}"
                                    })
